...
class BotCore {
    constructor() {
        ...
        // 🔥 CACHE DE NOMES DE GRUPO (para Context Isolation)
        this._groupNameCache = {};
    ...
    initializeComponents() {
        try {
            ...
            try {
                ...
                // ✅ Inicializa CommandHandler com SOCK para garantir StickerViewOnceHandler
                if (this.sock) {
                    this.commandHandler = new CommandHandler(this, this.sock); // ✅ CRÍTICO: Passa sock!
                    console.log('✅ CommandHandler inicializado com sock - StickerViewOnceHandler disponível');
                } else {
                    this.commandHandler = new CommandHandler(this);
                    console.warn('⚠️  CommandHandler inicializado SEM sock - Stickers não disponíveis ainda');
                }
            ...
            try {
                if (ehGrupo) {
                    ...
                    if (isLevelingEnabled) {
                        // ✅ Validações robustas antes de award XP
                        if (!this.levelSystem) {
                            this.logger.warn('⚠️ LevelSystem não inicializado - pulando XP');
                            return; // Early return se levelSystem não existe
                        }

                        const uid = m.key.participant || m.key.remoteJid;
                        if (!uid) {
                            this.logger.warn('⚠️ UID inválido - pulando XP');
                            return;
                        }
                        ...
                        try {
                            ...
                            // ✅ Log detalhado do erro específico de XP
                            this.logger.error('❌ Erro específico ao processar XP:', {
                ...
                // ✅ Log mais informativo do erro geral
                this.logger.warn('Erro awarding XP (outer catch):', {
                    message: e.message,
                    stack: e.stack?.substring(0, 200) // Limita stack trace
                });
            }
        ...
        try {
            ...
            if (foiAudio) {
                ...
                if (replyInfo && replyInfo.ehRespostaAoBot) {
                    ...
                    // Em PV: Responde sempre (com rate limit já verificado acima)
                    deveResponder = true;
                    this.logger.debug('📩 Mensagem PV detectada - autorizando resposta');
                } else {
            ...
            if (!deveResponder) {
                // 🎧 MODO ESCUTA / AUTONOMO
                // Se não respondeu diretamente, envia para análise de aprendizado e possível intervenção autônoma
                if (ehGrupo && texto.length > 3) {
                    // 🔥 FIX: Context Isolation no modo escuta também
                    const grupoId = m.key.remoteJid;
                    const grupoNome = await this._getGroupName(m.key.remoteJid);

                    // Prepara payload de escuta (mais leve)
                    const payloadEscuta = {
                        mensagem: texto,
                        usuario: nome,
                        numero: numeroReal,
                        nome_usuario: nome,
                        tipo_conversa: 'grupo',
                        contexto_grupo: grupoNome,
                        grupo_id: grupoId
                    };

                    // Fire-and-forget (mas com check de intervenção)
                    this.apiClient.request('POST', '/escutar', payloadEscuta)
                        .then(async (res) => {
                            if (res.success && res.data && res.data.intervention_suggested) {
                                this.logger.info(`🤖 [AUTONOMO] Intervenção decidida: ${res.data.intervention_reason}`);

                                // Força resposta chamando o processamento principal
                                // Pequeno delay para parecer natural
                                await delay(1500 + Math.random() * 2000);

                                const payloadMain = this.apiClient.buildPayload({
                                    usuario: nome,
                                    numero: numeroReal,
                                    mensagem: texto,
                                    tipo_conversa: 'grupo',
                                    ...
                                    reply_metadata: replyInfo ? {
                                        is_reply: replyInfo.isReply || false,
                                        reply_to_bot: false, // Não é reply ao bot, é intervenção
                                ...
                                // Chama API principal
                                const resultado = await this.apiClient.processMessage(payloadMain);
                                if (resultado.success) {
                                    // Envia resposta
                                    await this.simulateTyping(m.key.remoteJid, Math.min(resultado.resposta.length * 50, 4000));
                                    await this.sock.sendMessage(m.key.remoteJid, { text: resultado.resposta });
                                    this.logger.info(`✅ [AUTONOMO RESPONDIDO] ${resultado.resposta.substring(0, 50)}..`);
                                }
                            }
                        })
                        .catch(err => this.logger.debug(`Erro no /escutar: ${err.message}`));
                }
            ...
            // 🔥 CONTEXT ISOLATION FIX: Extrai grupo_id e grupo_nome
            const grupoId = ehGrupo ? m.key.remoteJid : null;
            const grupoNome = ehGrupo ? await this._getGroupName(m.key.remoteJid) : null;

            if (grupoId) {
                this.logger.debug(`📍 Grupo detectado: ${grupoNome} [${grupoId.substring(0, 20)}...]`);
            }
            ...
            const payload = this.apiClient.buildPayload({
                ...
                grupo_id: grupoId,        // 🔥 FIX: Context Isolation
                grupo_nome: grupoNome     // 🔥 FIX: Context Isolation
            });
    ...
    */
    ...
    /**
  * 🔥 OBTÉM NOME DO GRUPO COM CACHE (Context Isolation Fix)
  * Evita múltiplas chamadas à API do WhatsApp
  */
    async _getGroupName(groupJid) {
        try {
            // Verifica cache
            if (this._groupNameCache && this._groupNameCache[groupJid]) {
                return this._groupNameCache[groupJid];
            }

            // Busca metadata do grupo
            const metadata = await this.sock.groupMetadata(groupJid);
            const name = metadata?.subject || 'Grupo';

            // Cacheia
            if (!this._groupNameCache) this._groupNameCache = {};
            this._groupNameCache[groupJid] = name;

            this.logger.debug(`📝 Nome do grupo cacheado: ${name} [${groupJid.substring(0, 15)}...]`);
            return name;
        } catch (error) {
            this.logger.warn(`⚠️ Erro ao obter nome do grupo: ${error.message}`);
            return 'Grupo';
        }
    }
