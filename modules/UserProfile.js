...
class UserProfile {
  ...
  async getUserInfo(userJid) {
    try {
      ...
      // Tentar obter nome do grupo se disponível
      let nameInGroup = null;
      try {
        // Este método pode não estar disponível em todas as versões do Baileys
        if (this.sock.groupMetadata && typeof this.sock.groupMetadata === 'function') {
  ...
  extractUserJidFromMessage(message, m) {
    ...
    if (m && m.message && m.message.extendedTextMessage && 
