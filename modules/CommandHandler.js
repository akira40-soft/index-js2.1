...
💡 Ganhe XP conversando naturalmente no grupo!`;
            ...
            if (cmd === 'remove' || cmd === 'kick' || cmd === 'ban') {
                return await ownerOnly(async () => {
                    try {
                        ...
                        let targets = [];
                        if (m.message.extendedTextMessage.essage.extendedTextMessage.contextInfo.essage.extendedTextMessage.contextInfo.mentionedJid) {
            ...
            if (cmd === 'sticker' || cmd === 's' || cmd === 'fig') {
                try {
                    // ✅ Lazy initialization - tenta inicializar se ainda não existir
                    if (!this.stickerHandler && this.sock) {
                        console.log('🔄 Lazy init: Inicializando StickerViewOnceHandler...');
                        this.stickerHandler = new StickerViewOnceHandler(this.sock, this.config);
                        console.log('✅ StickerViewOnceHandler inicializado com sucesso (lazy)');
                    }

                    if (!this.stickerHandler) {
                        await sock.sendMessage(m.key.remoteJid, {
                            text: '❌ *STICKER INDISPONÍVEL*\n\nO handler de stickers ainda não foi carregado.\n\n💡 Tente novamente em alguns segundos ou entre em contato com o administrador.'
                        }, { quoted: m });
                        return true;
                    }
            ...
            if (cmd === 'play') {
                try {
                    ...
                    if (!this.mediaProcessor.isValidYouTubeUrl(full)) {
                        ...
                        if (!searchResult || !searchResult.resultados || searchResult.resultados.length === 0) {
                            await sock.sendMessage(m.key.remoteJid, {
                        ...
                        await sock.sendMessage(m.key.remoteJid, {
                            text: `🔍 Encontrado: *${searchResult.resultados[0].title}*\nBaixando...`
                        }, { quoted: m });
                    }
                    ...
                    if (!downloadResult || !downloadResult.sucesso) {
                        await sock.sendMessage(m.key.remoteJid, {
                            text: `❌ Erro ao baixar: ${downloadResult ? downloadResult.error : 'Erro desconhecido'}`
                        }, { quoted: m });
                        return true;
                    }

                    // Simula gravação (se available, senao ignora)
                    if (this.simulateRecording) {
                        await this.simulateRecording(m.key.remoteJid);
                    }

                    // Envia áudio
                    await sock.sendMessage(m.key.remoteJid, {
                        audio: downloadResult.buffer, // Buffer do arquivo
                        mimetype: 'audio/mpeg',
                        fileName: `${downloadResult.titulo}.mp3`,
                        ptt: false // Enviar como áudio normal, não nota de voz (para qualidade melhor)
                    }, { quoted: m });
                    ...
                    await sock.sendMessage(m.key.remoteJid, {
                        text: '❌ Ocorreu um erro interno ao processar o comando #play.'
                    }, { quoted: m });
                    return true;
                }
            }
...
// ═══════════════════════════════════════════════════════════════
// COMANDO BLACKLIST (MODERAÇÃO)
// ═══════════════════════════════════════════════════════════════

if (cmd === 'blacklist' || cmd === 'blocklist') {
    return await ownerOnly(async () => {
        try {
            const sub = (args[0] || '').toLowerCase();
            const modSystem = this.bot.moderationSystem;

            if (!modSystem) {
                await sock.sendMessage(m.key.remoteJid, { text: '❌ Sistema de moderação não inicializado.' }, { quoted: m });
                return true;
            }

            // Subcomandos: add, remove, list, check
            if (!['add', 'remove', 'list', 'check'].includes(sub)) {
                await sock.sendMessage(m.key.remoteJid, {
                    text: '🚫 *COMANDO BLACKLIST*\n\nUso:\n#blacklist add <numero> [motivo]\n#blacklist remove <numero>\n#blacklist list\n#blacklist check <numero>'
                }, { quoted: m });
                return true;
            }
            ...
            // LIST
            if (sub === 'list') {
                const report = modSystem.getBlacklistReport();
            ...
            // Tenta extrair JID de menção, reply ou argumento
            if (m.message?.extendedTextMessage?.contextInfo?.mentionedJid?.length) {
                targetJid = m.message.extendedTextMessage.contextInfo.mentionedJid[0];
            } else if (replyInfo && replyInfo.participantJidCitado) {
                targetJid = replyInfo.participantJidCitado;
            } else if (targetInput) {
                targetJid = `${targetInput.replace(/\D/g, '')}@s.whatsapp.net`;
            }

            if (!targetJid && sub !== 'list') {
                await sock.sendMessage(m.key.remoteJid, { text: '❌ Indique um usuário (menção, reply ou número).' }, { quoted: m });
                return true;
            }

            // ADD
            if (sub === 'add') {
                const reason = args.slice(2).join(' ') || 'Violação de regras';
                const success = modSystem.addToBlacklist(targetJid, 'User', targetInput || '?', reason);

                if (success) {
                    await sock.sendMessage(m.key.remoteJid, {
                        text: `✅ *USUÁRIO BLOQUEADO*\n\n👤 ${targetJid}\n📝 Motivo: ${reason}\n🚫 Usuário será ignorado pelo bot.`
                    }, { quoted: m });
                    // Tenta remover do grupo se for grupo
                    if (ehGrupo) {
                        try {
                            await sock.groupParticipantsUpdate(m.key.remoteJid, [targetJid], 'remove');
                            await sock.sendMessage(m.key.remoteJid, { text: '👋 Usuário removido do grupo.' });
                        } catch (e) {
                            console.warn('Não foi possível remover usuário blacklisted do grupo:', e);
                        }
                    }
                } else {
                    await sock.sendMessage(m.key.remoteJid, { text: '❌ Erro ao adicionar à blacklist (já existe?).' }, { quoted: m });
                }
            }

            // REMOVE
            else if (sub === 'remove') {
                const success = modSystem.removeFromBlacklist(targetJid);
                if (success) {
                    await sock.sendMessage(m.key.remoteJid, { text: `✅ Usuário ${targetJid} removido da blacklist.` }, { quoted: m });
                } else {
                    await sock.sendMessage(m.key.remoteJid, { text: '❌ Usuário não encontrado na blacklist.' }, { quoted: m });
                }
            }

            // CHECK
            else if (sub === 'check') {
                const isBlocked = modSystem.isUserBlacklisted(targetJid);
                await sock.sendMessage(m.key.remoteJid, {
                    text: isBlocked ? `🚫SIM: ${targetJid} está na blacklist.` : `✅ NÃO: ${targetJid} está limpo.`
                }, { quoted: m });
            }

        } catch (e) {
            console.error('Erro em blacklist:', e);
            await sock.sendMessage(m.key.remoteJid, { text: '❌ Erro ao processar blacklist.' }, { quoted: m });
        }
        return true;
    });
}
            ...
            if (!full || !full.includes(' ')) {
                await sock.sendMessage(m.key.remoteJid, {
                    text: `🔓 *HYDRA - REAL PASSWORD CRACKING*\n\n` +
                        ...
                        `Exemplo: #hydra 192 && 2 && 2.168.1.1:22 root password_list.txt\n\n` +
...
*3️⃣ #hydra <target> <user> <file>*
 ...
 ⏱️ Timeout: 30 min
 Exemplo: #hydra 192 && 2 && 2.168.1.1:22 root passwords.txt
