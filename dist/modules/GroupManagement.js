...
class GroupManagement {
     ...
     */
    async _waitForSocket(maxWaitMs = 5000) {
     ...
     */
    async _getGroupMetadata(groupJid, retries = 3) {
        // Verifica cache
    ...
    async addUser(m, args) {
        ...
        try {
            ...
            const success = result.filter((r) => r.status === 200);
            const failed = result.filter((r) => r.status !== 200);
            let message = '';
            if (success.length > 0) {
                const mentions = success.map((r) => `@${r.jid.split('@')[0]}`).join(', ');
