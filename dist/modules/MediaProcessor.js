...
class MediaProcessor {
     ...
     */
    async _runYtDlpWithFallback(buildCommand, expectedOutputPath, captureOutput = false) {
    ...
    async downloadYouTubeAudio(url) {
        try {
            ...
            const result = await this._runYtDlpWithFallback((bypassArgs) => `yt-dlp ${bypassArgs} -f "bestaudio[ext=m4a]/bestaudio/best" --extract-audio --audio-format mp3 --audio-quality 2 -o "${outputPath}" "${url}"`, outputPath);
    ...
    async downloadYouTubeVideo(url, quality = '720') {
        try {
            ...
            const result = await this._runYtDlpWithFallback((bypassArgs) => `yt-dlp ${bypassArgs} -f "${formatSelector}" --merge-output-format mp4 -o "${outputPath}" "${url}"`, outputPath);
            if (!result.sucesso || !fs.existsSync(outputPath)) {
                return { sucesso: false, error: result.error || 'Falha no download' };
            }
            const buffer = await fs.promises.readFile(outputPath);
    ...
    async getYouTubeMetadata(url) {
        try {
            ...
            if (!result.sucesso || !result.output) {
                result = await this._runYtDlpWithFallback((bypassArgs) => `yt-dlp ${bypassArgs} --dump-json --no-download "${url}"`, undefined, true);
            }
            if (!result.sucesso || !result.output) {
    ...
    async downloadMedia(message, mimeType = 'image') {
        try {
            ...
            const extractMediaContainer = (msgObj) => {
            ...
            for (let attempt = 1; attempt <= 3; attempt++) {
                try {
                    const stream = await downloadContentFromMessage(mediaContent, finalMimeType);
